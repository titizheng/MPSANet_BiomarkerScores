...
parser = argparse.ArgumentParser(description="Coordinate point patch ""from mask")

parser.add_argument("--patch_number", default=200, metavar="PATCH_NUMB", type=int,
...
class patch_point_in_mask_gen(object):
    ...
    def __init__(self, mask_path, number):
        self.mask_path = mask_path ##mask文件
        self.number = number
    ...
    def get_patch_point(self):
        ...
        centre_points = np.stack(np.vstack((X_idcs.T, Y_idcs.T)), axis=1) ##最后的形状，多一个维度


        if centre_points.shape[0] > self.number:
            sampled_points = centre_points[np.random.randint(centre_points.shape[0], size=self.number), :]
        else:
...
def run(args, mask_path, txt_path):
    sampled_points = patch_point_in_mask_gen(mask_path, args.patch_number).get_patch_point()
...
if __name__ == "__main__":
    ...
    mask_path_root =r"D:\zhengtingting222\2021-3-15code\CD30\data\heatimage\maks_W_JSON"  #
    txt_path_root =r"D:\zhengtingting222\2021-3-15code\CD30\data\heatimage\sample"

    mask_name='66650(CD30).npy'
    mask_path = os.path.join(mask_path_root, mask_name)
    print("1.mask的路径是:{}".format(mask_path))
