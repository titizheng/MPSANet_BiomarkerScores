...
class GridWSIPatchDataset(Dataset):
    ...
    def _preprocess(self):
        ...
        self._X_idcs2, self._Y_idcs2 = np.where(self._mask)##返回的是，所有mask中不为0的部分，也就是切的patch都是去除背景的部分
        self._idcs_num2 = len(self._X_idcs2)

        numbercount = int(self._idcs_num2 / 32)
        centre_points = np.stack(np.vstack((self._X_idcs2.T, self._Y_idcs2.T)), axis=1)  # 所有样本的坐标点
        sampled_points = centre_points[np.random.randint(0, len(centre_points), numbercount)]
        self._X_idcs, self._Y_idcs = sampled_points[: , 0] , sampled_points[:,1]
