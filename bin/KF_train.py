...
from data.image_producer import GridImageDataset
# from wsi.data.image_producer import GridImageDataset  # noqa
from wsi.model import MODELS, MODELS_vgg, MODELS_resnet #,MODELS_Efficient # noqa
...
parser.add_argument('--cfg_path', default=r"/home/omnisky/hdd_15T_sdc/zttcode/cd-30NCRF-KF2/cd-30NCRF-KF/configs/TCGApre_model_crf.json", metavar='CFG_PATH', type=str,
                    help='Path to the config file in json format')
parser.add_argument('--save_path', default=r"/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/KF-result", metavar='SAVE_PATH', type=str,
                    help='Path to the saved models')

parser.add_argument('--num_workers', default=0, type=int, help='number of'
                    ' workers for each data loader, default 2.') ##把进程数设置为0，这样防止内存不足
...
def train_epoch(summary, summary_writer, cfg, model, loss_fn, optimizer,
    ...
    dataiter_tumor = iter(dataloader_tumor)  ##有病的部分
    ...
    for step in range(steps):
        ...
        # print('Output tumor tags',target_tumor)
        # print('Output normal tags', target_normal)
...
####---------------------------------------------要在这个地方使用交叉验证-------------------------------
##说明：dataset_tumor_train和dataset_normal_train拆成训练集和验证集
    turmor_path=np.array(os.listdir(cfg['data_path_tumor_train']))  ##只有数组的形式，下面的用turmor_path[train_index]这种形式取才不会出错
    normal_path=np.array(os.listdir(cfg['data_path_normal_train'])) ##数据的地址
    kf = KFold(n_splits=5, shuffle=True)
    kf.get_n_splits(turmor_path, normal_path) ##这个就当成两个数据集，以normal_path为基准分下标，因为normal_path的个数少
    i=1
    ...
    for train_index, val_index in kf.split(normal_path):
        print('输出i的值',i)
        print('*******************************第{}次交叉验证,训练集数据长度{},验证集数据长度{}************************'.format(i,len(train_index),len(val_index)))
        tumor_train,normal_train=turmor_path[train_index],normal_path[train_index] ##现在这个里面存的都是图片名字
    ...
    ####-----------------------------------------画出来交叉验证的图-------------------------
        ...
        tprs.append(np.interp(mean_fpr, fpr, tpr)) ##为了用来求均值用的
        tprs[-1][0] = 0.0
        # 计算auc
        roc_auc = auc(fpr, tpr)
        aucs.append(roc_auc)
...
####--------------------------在主坐标图上嵌入子坐标系---------------------
    axins = inset_axes(ax, width="30%", height="30%", loc='lower left',
                       ...
                       bbox_transform=ax.transAxes) #center left bbox_to_anchor=(0.5, 0.1, 1, 1)

    # print(np.array(x1).shape)##这个形状是3

    ##存储每一次交叉验证的fpr, tpr,
    # print(x1)
    f_fpr= open(r'/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/KF-resultl\fpr.txt', mode='w')
    f_fpr.write(str(x1))  # write 写入
    f_fpr.close()

    f_tpr = open(r'/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/KF-result\tpr.txt', mode='w')
    f_tpr.write(str(y1))  # write 写入
    f_tpr.close()




    for x ,y in zip(x1,y1): ##在子坐标系同画出原始图像
    ...
    plt.savefig(r'/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/KF-result/model_ROC.png')
    plt.show()
