...
parser.add_argument('--ckpt_path', default=r'/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/KF-resultxin/train_train.ckpt', metavar='', type=str,
...
parser.add_argument('--save_path', default=r"/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/testresult", metavar='SAVE_PATH', type=str,
...
def valid_epoch(summary, cfg, model,
    ...
    fp = open(os.path.join('/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/testresult', 'filename' + '.csv'), 'w')
    fp.write('target,prediction\n')
    fp.close()

    acc_sum = 0
    y_true=[] ##模型的真实标签
    probs_gl=[]##存放模型输出结果的概率值
    predicts_lable=[] ##模型预测的标签值
    for step in range(steps):
        ...
        problist4 = np.around(probs1, 4)
        probs_gl.append(probs1)
        # print('输出模型预测的概率值',probs1)

        ##------------------------------模型输出的标签------------
        # print('输出模型预测的结果', probs)
        with open(os.path.join('/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/testresult', 'filename' + '.csv'), 'a',  newline='') as fp:
            for tar1,  prob1 in zip( target1, problist4):  # 预测的值，用来输出slide的标签
                fp.write('{},{}\n'.format(tar1, prob1))
            fp.close()

        predicts = (probs >= 0.5).type(torch.cuda.FloatTensor) ##把大于0.5的都赋值为1，小于0.5的赋值为0
        predicts1 = predicts.cpu().numpy()
        predicts_lable.append(predicts1)
        # print('输出模型预测的结果', predicts1)
    ...
    # plt.savefig(r"D:\zhengtingting222\2021-3-15code\CD30\cd-30NCRF-KF\cd-30NCRF-KF\result/hunxiaojuzhen.png")
    plt.show()
  ...
  ##------------------------------
    ...
    plt.title('ROC')
    plt.legend(loc='best')
    plt.savefig(r"/home/omnisky/hdd_15T_sdc/zttdata/TCGA/TCGAHEHER2/testresult/testresult/ROC_2.png")
    plt.show()
