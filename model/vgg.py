...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.features(x) #输入层，代表输入的特征
        x = self.avgpool(x)
        # x = torch.flatten(x, 1)
        # x = self.classifier(x)

        ####如果是注意力机制，不用，加上全连接层
        #------------------------------------原本的部分，能跑CRF的部分，这一部分加上CRF就是CRF的部分-------------------
        ...
        feats = attention_add.view(-1, 512*7*7)#288,512，
        logits = self.classifier(feats)
        logits = logits.view((batch_size, grid_size, -1))
        feats = feats.view((batch_size, grid_size, -1))
...
def _vgg(arch, cfg, batch_norm, pretrained, progress, **kwargs):
    ...
    # ckpt = torch.load(r'D:\zhengtingting222\2021-3-15code\CD30\cd-30NCRF-KF\cd-30NCRF-KF\configs\val_best.ckpt', map_location='cpu')
    ...
    if pretrained: ##这一部分才是加载预训练的参数
        ...
        for k in new_state_dict.keys():
            ...
            if k in dd.keys() and not k.startswith('classifier'):  # 去掉classifier层中的参数，startswith是查找含有某个字符的数值
                print('yes')
                dd[k] = new_state_dict[k] ##用已经训练好的模型参数，替换掉，dd中初始化的部分参数。存储的是字典形式
        # print('输出dd',dd)
