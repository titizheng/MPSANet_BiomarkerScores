...
class CRF(nn.Module):
    def __init__(self, num_nodes, iteration=20): #开始是10
        ...
        self.W = nn.Parameter(torch.zeros(1, num_nodes, num_nodes))  ##初始化权重参数，设置可训练的参数
    ...
    def forward(self, feats, logits):
        ...
        例如余弦相似度来导出成对的势。如果(x_i, x_j)相似，用余弦相似度pairwise_sim来衡量，成对势会鼓励用相同的标签分配(y_i, y_j)得到高回报
        ...
        """
        feats_norm = torch.norm(feats, p=2, dim=2, keepdim=True) #返回一个给定张量的矩阵范数或向量范数。
        ...
        for i in range(self.iteration):
            ...
            probs = torch.transpose(logits.sigmoid(), 1, 2)  ##这个是0或者1
