...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        layers: List[int],
        num_classes=1, num_nodes=1,use_crf=True,
        ...
        self.fc = nn.Linear(512 * block.expansion, num_classes) ##改变了这个原本全连接的参数，所以这个地方之前的权重舍弃
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.avgpool(x)
        feats = x.view(x.size(0), -1) ##feats是288,512，x是288,512,1,1
        ...
        # feats = feats.view((batch_size, grid_size, -1)) ##32,9,512
        # # logits = logits.view((batch_size, grid_size, -1))
        # feats_norm = torch.norm(feats, p=2, dim=2, keepdim=True)  # 32,9,1
        # pairwise_norm = torch.bmm(feats_norm,
        #                           torch.transpose(feats_norm, 1, 2))# 32,9,9
        # pairwise_dot = torch.bmm(feats, torch.transpose(feats, 1, 2))# 32,9,9
        # # cosine similarity between feats
        # pairwise_s = pairwise_dot / pairwise_norm ## 32,9,9
        # attention_W=pairwise_s.softmax(dim=2) ## 32,9,9
        # # attention_W=pairwise_s/torch.sum(pairwise_s)
        # # print('attention的形状',attention_W)
        #
        # attention_MU=torch.bmm(attention_W, feats) #32,9,512
        # attention_add=torch.add(attention_MU,feats)#32,9,512
        #
        # ##把注意力机制的结果作为网络的是输出
        # feats = attention_add.view(-1, 512)#288,512，变成这个是为了输入到最后一个全连接，全连接之后可以暑促
        # logits = self.fc(feats) ##这个当做注意力机制之后的输出
        ...
        # ##输入到CRF中
        # feats = feats.view((batch_size, grid_size, -1))  ##size,9,
        # logits = logits.view((batch_size, grid_size, -1))
        # if self.crf:
    ...
    **kwargs: Any
    ...
    if pretrained:
        ...
        for k in new_state_dict.keys():
            # print(k)
            if k in dd.keys() and not k.startswith('fc'):  # 去掉classifier层中的参数，startswith是查找含有某个字符的数值
                # print('yes')
